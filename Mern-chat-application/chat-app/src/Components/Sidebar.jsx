                            ...
                            <div onClick={() => setSelectedUser(user)} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm
                             ${selectedUser?._id == user._id && 'bg-[#282142]/50'}`}>
                                ...
                                <div className="flex flex-col leading-5">
                                    <p>{user.fullName}</p>
                                    { 
                                        onlineUser.includes(users._id) ? <span className="text-green-400 text-xs">Online</span>
                                ...
                                {unseenMessages[users._id] > 0 && <p className="absolute top-4 right-4 text-xs h-5 w-5
                                 flex justify-center items-center rounded-full bg-violet-500/50">{unseenMessages[users._id]}</p>}
                            </div>
                        )
                    )
                    }
